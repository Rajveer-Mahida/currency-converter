...
function App() {
  const [amount, setaAmount] = useState(0);
          ...
          >
            <div className='w-full mb-1'>
              <InputBox
                ...
                onCurrncyChange={(currency) => {
                 setFrom(currency)
                }}
                onAmoutChange={(amount)=>{setaAmount(amount)}}
                selectCurrency={from}
              />
            </div>
            <div className='relative w-full h-0.5'>
              <button
                type='button'
                className='absolute left-1/2 -translate-x-1/2 -translate-y-1/2 border-2 border-white rounded-md bg-blue-600 text-white px-2 py-0.5 font-bold'
                onClick={()=>{swap()}}
              >
                Swap
              </button>
